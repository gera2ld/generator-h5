...
exports.addWebpackConfig = (ins, addConfig, addOptions) => {
  ...
  let content = source;
  if (addConfig)  content = content.replace('/** WEBPACK_CONFIG **/', m => `${addConfig}${m}`);
  if (addOptions) content = content.replace('/** WEBPACK_OPTIONS **/', m => `${addOptions}${m}`);
  ins.fs.write(filePath, content);
};
