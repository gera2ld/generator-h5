...
exports.addWebpackConfig = (ins, replacements = {}) => {
  ...
  let content = source;
  const { prepare, plugins, options, modifications } = replacements;
  if (prepare) content = content.replace('/** WEBPACK_PREPARE **/', m => `${prepare}${m}`);
  if (plugins) content = content.replace('/** WEBPACK_PLUGINS **/', m => `${plugins}${m}`);
  if (options) content = content.replace('/** WEBPACK_OPTIONS **/', m => `${options}${m}`);
  if (modifications) content = content.replace('/** WEBPACK_MODIFICATION **/', m => `${modifications}${m}`);
  ins.fs.write(filePath, content);
};
