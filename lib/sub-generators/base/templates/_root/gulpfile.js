...
const DIST = require('webpack-util/util').defaultOptions.distDir;
const webpackConfig = require('./scripts/webpack.conf');

const COPY = <%= features.includes('copy') %>;
...
function watch() {
  if (!COPY) return Promise.resolve();
  return gulp.watch('src/public/**', copyPublic);
}
